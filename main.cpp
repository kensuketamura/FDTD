...
void modeling(){
    ...
    int x2,y2;
	int Pre_Row, Re_Row,Pre_Cell, Re_Cell;
	...
	int Row_x;
	if(irank != IRANK_MAX){
		Pre_Row = INT_DIV (irank*(XMAX-1), intPitchX);
		Re_Row = INT_DIV ((irank+1)*(XMAX-1), intPitchX)+1;

		Pre_Cell = irank*(XMAX-1) - intPitchX*Pre_Row;
		Re_Cell = intPitchX*Re_Row - (irank+1)*(XMAX-1);

		Row_x = Re_Row - Pre_Row +1;

		for(x2 = 0; x2 < Row_x; x2++){		
			for(y2 = 0; y2 < Row_y; y2++){
				if(y2 % 2 == 0){
					x = intPitchX * x2 + intPitchX * 0.5 - Pre_Cell;
					y = ymax - Wid - intPitchY*y2;
				}else{
					x = intPitchX * x2 - Pre_Cell;
					y = ymax - Wid - intPitchY*y2;
				}
				for(z = 0; z < zmax+1; z++){
					mcircle(x, y, z);				
	...
	if(irank != IRANK_MAX){
		for(x = 0; x < xmax+1; x++){
			for(z = 0; z < zmax+1; z++){
				cell[x][ymax][z] = cell[x][ymax-1][z]; 
				epsilonx[x][ymax][z] = epsilonx[x][ymax-1][z]; 
				epsilony[x][ymax][z] = epsilony[x][ymax-1][z]; 
				epsilonz[x][ymax][z] = epsilonz[x][ymax-1][z]; 
			}
		}

		for(x = 0; x < xmax+1; x++){
			for(y = 0; y < ymax+1; y++){
				cell[x][y][zmax] = cell[x][y][zmax-1]; 
				epsilonx[x][y][zmax] = epsilonx[x][y][zmax-1]; 
				epsilony[x][y][zmax] = epsilony[x][y][zmax-1]; 
				epsilonz[x][y][zmax] = epsilonz[x][y][zmax-1]; 
			}
		}
	}else{
		for(x = 0; x < xmax+1; x++){
...
void mcircle(int x_circ, int y_circ, int z_circ){
	...
	if(x_circ == 0){
	rightquartercircle1(x_circ+1, y_circ, z_circ, R); 
	leftquartercircle1(x_circ, y_circ, z_circ, R); 
	rightquartercircle2(x_circ+1, y_circ-1, z_circ, R); 
	leftquartercircle2(x_circ, y_circ-1, z_circ, R);
	}else{
